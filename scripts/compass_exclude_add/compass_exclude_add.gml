/// @function compass_exclude_add(instance_or_array)
...
/// @return {undefined}
function compass_exclude_add(_id_or_array){
	if ( _id_or_array == undefined ) return;
	if ( !is_array(_id_or_array) ) _id_or_array = [_id_or_array];
	__Compass().__exclude = array_concat(__Compass().__exclude, _id_or_array);
}